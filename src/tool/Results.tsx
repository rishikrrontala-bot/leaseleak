...
export default function Results({ analysis: a, fileName, cap, onCap, utilities, onUtilities, onReset }: Props) {
  ...
  return (
    <div ref={root} className="mx-auto w-full max-w-6xl">
      {/* ---- Header ---- */}
      <div className="mb-10 flex flex-wrap items-end justify-between gap-4 border-b border-paper/15 pb-6">
        <div>
          <p className="t-micro uppercase tracking-[0.14em] text-paper/60">Rent roll</p>
          <h2 className="t-h3 mt-1">{fileName}</h2>
          <p className="t-small mt-1 text-paper/60">
      ...
      <section className="mt-16" aria-labelledby="units">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="t-body mt-2 text-paper/70">Current rent against the HUD benchmark. Ember is the gap.</p>
          </div>
          <div role="tablist" aria-label="View" className="flex rounded-full border border-paper/20 p-1 t-small">
            {(['gap', 'timing'] as const).map((v) => (
              <button
                key={v} role="tab" aria-selected={view === v} type="button"
                className={`rounded-full px-4 py-1.5 font-medium transition-colors duration-150 ${view === v ? 'bg-paper text-ink' : 'text-paper/70'}`}
                onClick={() => setView(v)}
              >
                    ...
                    <li key={u.rowIndex} className="grid grid-cols-[4.5rem_1fr_auto] items-center gap-3 sm:grid-cols-[6rem_1fr_11rem]">
                      ...
                      <div className="t-small text-right num">
                        {view === 'gap' ? (
                          ...
                          u.leaseEnd ? (
                            <><span className={`font-semibold ${(u.timingValueAnnual ?? 0) > 100 ? 'text-ember-3' : 'text-mint'}`}>{(u.timingValueAnnual ?? 0) > 100 ? `${money(u.timingValueAnnual!)}/yr` : 'Well timed'}</span><span className="block text-paper/60">ends {MONTHS[u.expiryMonth!]} · best {MONTHS[u.bestMonth]}</span></>
                          ) : <span className="text-paper/50">No lease end</span>
                        )}
                      </div>
                    </li>
                  ))}
      ...
      <section className="mt-20" aria-labelledby="expiries">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5">
            <p className="t-micro mb-3 uppercase tracking-[0.14em] text-paper/60">Next 90 days</p>
            {a.expiring90.length === 0 ? (
              <p className="t-body text-paper/70">No leases end in the next 90 days.</p>
            ) : (
              <ul className="divide-y divide-paper/10 rounded-2xl bg-field-2">
                {a.expiring90.map((u) => (
                  <li key={u.rowIndex} className="flex items-center justify-between gap-4 px-5 py-4">
                    <div>
                      <p className="font-semibold">Unit {u.id} <span className="font-normal text-paper/60">· {u.property}</span></p>
                      <p className="t-small text-paper/70">Ends {fmtDate(u.leaseEnd!)} · {u.tenant ?? 'Resident'}</p>
                    </div>
                    <div className="text-right num">
                      <p className={`font-semibold ${u.daysToExpiry! <= 60 ? 'text-ember-3' : ''}`}>{u.daysToExpiry} days</p>
                      {u.suggestedIncrease ? <p className="t-small text-paper/70">renew at {money(u.suggestedRent!)}</p> : <p className="t-small text-mint">renew as is</p>}
                    </div>
                  </li>
                ))}
              </ul>
      ...
      {/* ---- Method ---- */}
      <section className="mt-20 grid gap-8 border-t border-paper/15 pt-10 md:grid-cols-12" aria-labelledby="method">
        ...
        <div className="t-body measure space-y-4 text-paper/80 md:col-span-8">
          <p>
            For every unit we look up the U.S. Department of Housing and Urban Development's <strong className="text-paper">Small Area Fair Market Rent</strong> for its ZIP code and bedroom count (FY{a.fy}, 38,601 ZIPs, 0–4+ bedrooms).
          ...
          <p>
            <strong className="text-paper">This is a conservative floor.</strong> Fair Market Rent is the 40th percentile of gross rents in the area — six in ten comparable homes rent for more than this — and it includes utilities. If your rent is below it, you are almost certainly below market.
          </p>
          <p>
...
function Stat({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="rounded-2xl bg-field-2 px-5 py-4">
      <p className="t-micro uppercase tracking-[0.14em] text-paper/60">{label}</p>
      <p className="t-h2 num mt-1">{value}</p>
      <p className="t-small mt-1 text-paper/60">{sub}</p>
    </div>
  );
}
...
function GapBar({ u, max }: { u: UnitResult; max: number }) {
  ...
  return (
    <div className="relative h-7 rounded-md bg-field-2" aria-label={`Rent ${money(u.rent)}, benchmark ${u.fmr !== null ? money(u.fmr) : 'unknown'}`}>
      ...
      {/* benchmark tick */}
      {u.fmr !== null && <div className="absolute inset-y-[-3px] w-[2px] bg-paper" style={{ left: `calc(${fmrW}% - 1px)` }} title={`HUD benchmark ${money(u.fmr)}`} />}
    </div>
  );
}
  ...
  return (
    <div className="flex h-7 items-end gap-[3px]" aria-label={u.expiryMonth !== null ? `Lease ends in ${MONTHS_LONG[u.expiryMonth]}` : 'No lease end date'}>
      {u.seasonal.map((v, i) => {
        ...
        return <div key={i} className={`flex-1 rounded-sm ${isEnd ? 'bg-ember' : isBest ? 'bg-mint-2' : 'bg-paper/20'}`} style={{ height: `${h}%` }} title={`${MONTHS[i]} ${v.toFixed(3)}`} />;
      })}
    </div>
  );
}
...
function SeasonChart({ months, curve }: { months: { label: string; m: number; count: number; units: UnitResult[] }[]; curve: number[] }) {
  ...
  return (
    <figure className="rounded-2xl bg-field-2 p-4 sm:p-6">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Seasonal rent index with lease expirations by month">
        <line x1={padX} x2={W - padX} y1={padTop + (1 - (1 - min) / (max - min || 1)) * (H - padTop - padBot)} y2={padTop + (1 - (1 - min) / (max - min || 1)) * (H - padTop - padBot)} stroke="rgb(244 239 227 / 0.2)" strokeDasharray="3 4" />
        ...
        {months.map((mo, i) => (
          <g key={i}>
            ...
            <text x={xs[i]} y={H - 14} textAnchor="middle" fontSize="12" fill="rgb(244 239 227 / 0.65)">{mo.label}</text>
          </g>
        ))}
      </svg>
      <figcaption className="t-small mt-2 flex flex-wrap gap-4 text-paper/60">
        ...
        <span><span className="mr-1 inline-block h-2 w-3 rounded-sm bg-paper align-middle" />Leases ending in an above-average month</span>
      </figcaption>
    </figure>
  );
}
