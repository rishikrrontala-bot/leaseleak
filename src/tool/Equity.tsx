    ...
    <section className="mt-16" aria-labelledby="equity">
      <div className="grid gap-8 md:grid-cols-12 md:gap-6">
        <div className="md:col-span-7">
          ...
          <div className="mt-3">
            <p key={`${a.totalGapAnnual}-${capRate}`} className="wipe-up-now t-display-sm num text-ink">{money(fullValue)}</p>
          </div>
        ...
        <div className="grid gap-4 self-end sm:grid-cols-2 md:col-span-5 md:grid-cols-1">
          <div className="rounded-2xl bg-canvas-2 px-5 py-4">
            <p className="t-micro uppercase tracking-[0.14em] text-ink/60">Added this renewal cycle</p>
            <p className="t-h2 num mt-1">{money(cycleValue)}</p>
            <p className="t-small mt-1 text-ink/60">if the increases capped at {fmtPct(cap, 0)} hold through the year</p>
          </div>
          <div className="rounded-2xl bg-canvas-2 px-5 py-4">
            <p className="t-micro uppercase tracking-[0.14em] text-ink/60">Per dollar of rent</p>
            <p className="t-h2 num mt-1">{fmtUSD(perDollar, { cents: true })}</p>
            <p className="t-small mt-1 text-ink/60">of value for each $1/yr of income at a {fmtPct(capRate, 2)} cap rate</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 rounded-2xl bg-canvas-2 px-5 py-4">
        ...
        <span className="t-small text-ink/60">Small multifamily typically trades at 5–8%. A lower cap rate means each dollar of rent is worth more. Assumes the added rent flows through to NOI.</span>
      </div>
    </section>
  );
}
