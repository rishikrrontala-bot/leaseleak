...
export default function Clustering({ analysis: a, curve }: { analysis: Analysis; curve: number[] }) {
  ...
  return (
    <div className="mt-10 grid gap-8 rounded-2xl bg-canvas-2 p-5 sm:p-6 lg:grid-cols-12">
      <div className="lg:col-span-5">
        <p className="t-micro uppercase tracking-[0.14em] text-ink/60">Concentration</p>
        <p className="t-h3 mt-2">
          {peakUnits} of {withEnd} leases end in {MONTHS_LONG[c.peak.m]}{concentrated ? ' — that’s a lot at once.' : '.'}
        </p>
        <p className="t-body mt-3 text-ink/75">
          That's <span className="num font-semibold text-ink">{fmtPct(c.peak.share, 0)}</span> of your monthly income up for renewal in one month.
...
function Rows({ label, counts, months, max, tone, className = '' }: { label: string; counts: number[]; months: { label: string; share: number }[]; max: number; tone: string; className?: string }) {
  return (
    <div className={className}>
      <p className="t-micro mb-2 uppercase tracking-[0.14em] text-ink/60">{label}</p>
      <div className="grid grid-cols-12 items-end gap-1" style={{ height: 64 }} role="img" aria-label={`${label}: leases ending per month`}>
        {counts.map((n, i) => (
          <div key={i} className="flex h-full flex-col justify-end">
            {n > 0 && <span className="num t-micro mb-0.5 text-center text-ink/70">{n}</span>}
            <div className={`w-full rounded-sm ${n ? tone : 'bg-ink/10'}`} style={{ height: n ? `${Math.max(8, (n / max) * 44)}px` : 2 }} />
          </div>
        ))}
      </div>
      <div className="mt-1 grid grid-cols-12 gap-1">
        {months.map((m, i) => <span key={i} className="t-micro text-center text-ink/50">{m.label}</span>)}
      </div>
    </div>
  );
}
