    ...
    <section className="mt-20" aria-labelledby="cashflow">
      <div className="grid gap-8 md:grid-cols-12 md:gap-6">
        ...
        <div className="self-end md:col-span-5">
          ...
          <p key={gainProposed} className="wipe-up-now t-display-sm num mt-2 text-ink">+{money(gainProposed)}</p>
          <p className="t-small mt-2 text-ink/70">with renewals capped at {fmtPct(cap, 0)} · +{money(gainBenchmark)} if every renewal went to the benchmark</p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <Total label="Do nothing" value={cf.totals.current} tone="bg-ink/40" />
...
function Total({ label, value, tone, delta }: { label: string; value: number; tone: string; delta?: number }) {
  return (
    <div className="rounded-2xl bg-canvas-2 px-5 py-4">
      <p className="t-micro flex items-center gap-2 uppercase tracking-[0.14em] text-ink/60"><span className={`inline-block h-2 w-4 rounded-sm ${tone}`} />{label}</p>
      <p className="t-h3 num mt-1">{money(value)}<span className="t-small font-normal text-ink/60"> / 12 mo</span></p>
      {delta !== undefined && <p className={`t-small num mt-0.5 ${delta > 0 ? 'text-mint' : 'text-ink/60'}`}>{delta > 0 ? '+' : ''}{money(delta)}</p>}
    </div>
  );
}
...
function Chart({ cf }: { cf: ReturnType<typeof cashflowProjection> }) {
  ...
  const labels = ([['benchmark', 'var(--color-mint-2)'], ['proposed', 'var(--color-ember)'], ['current', 'rgb(26 26 26 / 0.6)']] as const)
  ...
  return (
    <figure className="mt-8 rounded-2xl bg-canvas-2 p-4 sm:p-6">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Projected monthly income for the next twelve months under three renewal policies">
        {[0.25, 0.5, 0.75].map((t) => (
          <line key={t} x1={padL} x2={W - padR} y1={padTop + t * (H - padTop - padBot)} y2={padTop + t * (H - padTop - padBot)} stroke="rgb(26 26 26 / 0.08)" />
        ))}
        <path d={path('current')} fill="none" stroke="rgb(26 26 26 / 0.5)" strokeWidth="2" strokeDasharray="4 4" strokeLinejoin="round" />
        ...
        {cf.months.map((m, i) => (
          <text key={i} x={x(i)} y={H - 10} textAnchor={i === 0 ? 'start' : i === 11 ? 'end' : 'middle'} fontSize="12" fill="rgb(26 26 26 / 0.65)">{m.label}</text>
        ))}
        {labels.map((l) => (
      ...
      <figcaption className="t-small mt-2 flex flex-wrap gap-4 text-ink/60">
        <span><span className="mr-1 inline-block h-0.5 w-4 border-t-2 border-dashed border-ink/50 align-middle" />Do nothing</span>
