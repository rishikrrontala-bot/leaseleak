...
const GRADE_TONE: Record<Grade, string> = {
  A: 'text-mint', B: 'text-mint', C: 'text-paper', D: 'text-ember-3', F: 'text-ember',
};
  ...
  return (
    <section className="mt-20" aria-labelledby="buildings">
      ...
      <ol className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((c, i) => (
          <li key={c.property} className="grid grid-cols-[4.5rem_1fr] gap-4 rounded-2xl bg-field-2 p-5">
            <div className="text-center">
              <p className={`t-display-sm num leading-none ${GRADE_TONE[c.grade]}`}>{c.grade}</p>
              <p className="t-micro mt-2 text-paper/50">#{i + 1}</p>
            </div>
            <div className="min-w-0">
              <h4 className="t-h3 truncate">{c.property}</h4>
              <p className="t-small text-paper/60">{c.city && c.state ? `${c.city}, ${c.state} · ` : ''}ZIP {c.zip} · {c.units.length} units</p>
              <p className="num mt-3 text-[1.35rem] font-semibold leading-tight">
                ...
                <span className="t-small font-normal text-paper/60"> per door, per year</span>
              </p>
              ...
              {c.momentum && (
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <span className="t-small text-paper/60">{c.momentum.advice}</span>
                </div>
              )}
            </div>
          </li>
