    ...
    <section className="mt-20" aria-labelledby="vouchers">
      <div className="grid gap-8 md:grid-cols-12 md:gap-6">
        ...
        <div className="self-end md:col-span-5">
          ...
          <p key={`${v.upsideAnnual}`} className="wipe-up-now t-display-sm num mt-2 text-ink">{money(v.upsideAnnual)}</p>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 rounded-2xl bg-canvas-2 px-5 py-4">
        ...
        <span className="t-small text-ink/60">Each housing authority sets its standard between 90% and 110% of FMR. Look yours up, or leave it at 100%.</span>
      </div>

      {v.candidates === 0 ? (
        <p className="t-body mt-8 text-ink/70">Every matched unit already rents at or above the payment standard — no voucher upside at this setting.</p>
      ) : (
        <>
          <ul className="mt-8 divide-y divide-ink/10 rounded-2xl bg-canvas-2">
            {rows.map(({ unit: u, standard, upsideMonthly, upsideAnnual }) => (
              <li key={u.rowIndex} className="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-4 sm:grid-cols-[1fr_11rem_8rem]">
                <div>
                  <p className="font-semibold">Unit {u.id} <span className="font-normal text-ink/60">· {u.property ?? 'Portfolio'}</span></p>
                ...
                <p className="t-small num hidden text-ink/70 sm:block">
                  {money(u.rent)} <span className="text-ink/40">→</span> {money(standard)}
                  <span className="block text-ink/50">current → standard</span>
                </p>
                <div className="text-right num">
                  <p className="font-semibold text-ember-3">+{money(upsideAnnual)}/yr</p>
                  <p className="t-small text-ink/70">+{money(upsideMonthly)}/mo</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <div className="t-small measure mt-6 space-y-2 text-ink/60">
        <p>
          <strong className="text-ink/80">Best used at turnover.</strong> When one of these leases ends and the tenant moves on, list the unit as voucher-welcome at the payment standard. The authority inspects the unit and checks the rent is reasonable against comparable unassisted homes nearby.
        </p>
        <p>
