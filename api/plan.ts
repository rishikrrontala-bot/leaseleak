...
1. Use ONLY numbers, unit IDs, properties and dates that appear in the brief. Never invent, round differently, or extrapolate a figure. If you want to cite a number, copy it exactly as it appears.
...
6. Write in the second person ("you"), no exclamation marks, no marketing.`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const out = await gemini<PlanOut>({ system: SYSTEM, user: `BRIEF (JSON):\n${JSON.stringify(body.brief)}`, schema: SCHEMA, temperature: 0.25, maxOutputTokens: 1800 });
